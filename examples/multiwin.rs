...
use druid::{
    Action, AppLauncher, BaseState, BoxConstraints, Command, Data, Env, Event, EventCtx, LayoutCtx,
...
fn ui_builder() -> impl Widget<State> {
    ...
    let inc_button = Button::<State>::new("Add menu item").with_action(|ctx, data, _env| {
        data.menu_count += 1;
        let cmd = Command::new(druid::command::sys::SET_MENU, make_menu::<State>(data));
        ctx.submit_command(cmd, None);
        None
    });
    let dec_button = Button::<State>::new("Remove menu item").with_action(|ctx, data, _env| {
...
impl<T: Data> Widget<T> for EventInterceptor<T> {
    ...
    fn event(
        ...
        match (f)(event, ctx, data, env) {
            Some(event) => inner.event(&event, ctx, data, env),
            None => None,
        }
    }
